<template>
		<section>
			<div class="w-100 pa2 bg-light-gray br4 cf inline-flex items-center relative">
				...
				<router-link :to="{'name':'voters-new'}" class="fl right-1 absolute ph2 pv1 br4 bg-red hover-bg-dark-red grow-ns pointer f6 white tc no-underline">
					+ New
				</router-link>
			</div>
			...
			<table-display searchBy="voter name">
				...
				<template slot="tableBody" v-if="recordList.length > 0">
					<tr class="stripe-dark" v-for="(voter, index) in recordList" :key="index">
						<td class=" pa2 tl pr2">
			...
			<div v-if="recordList.length < 1">
				<p class="black f6 tc ma5">There are currently no voters within the system</p>
			</div>
