<template>
	<section>
		<div class="w-100 pa2 bg-light-gray br4 cf inline-flex items-center relative">
			...
			<router-link :to="{name:'voters-view',params:{id:record.ID}}" class="fl right-1 absolute ph2 pv1 br4 bg-red hover-bg-dark-red grow-ns pointer f6 white tc no-underline">
				Go Back
			</router-link>
		</div> 
		...
		<div v-if="isFound" class="pa2 bg-light-gray br4 cf">
			...
			<div class="w-100">
				<div class="fl pa2 mt3 mt0-ns w-100 w-50-ns">
					<label class=" fw4 lh-copy f6 black"> Voter</label>
					<label class=" fw4 lh-copy f7 red fr" @click="profileList=[],record.Voter='',record.VoterID=0">- clear</label>
					<input class="pa2 ba b--silver br2 bg-white w-100 " @keyup="searchVoter" type="text" v-model="record.Voter">
					<div class="relative w-100">
						<small>
							<ul class="bg-white absolute w-100 pa0 br2 br--bottom mt0 list">
								<li class="pa2 black bt b--near-white" @click="record.VoterID = voter.ID, record.Voter=voter.Fullname, voterList = []" v-for="(voter, index) in voterList" :key="index">
									<span class="f7">#{{index+1}}</span> {{voter.Fullname}}
								</li>
							</ul>
						</small>
					</div>
  ...
  export default {
	...
	methods: {
		searchVoter() {
			...
			const search = {text: app.record.Voter, field: "Fullname", limit: 20, skip: 0};
			HTTP.post(url, search,{withCredentials: true}).then((response) => {
				app["voterList"] = response.data.Body
			}).catch((e) => { console.log(e) })
		},
		searchPosition() {
			const app = this;
			const url = "/api/positions/search";
			const search = {text: app.record.Voter, field: "Title", limit: 20, skip: 0};
			HTTP.post(url, search,{withCredentials: true}).then((response) => {
		...
		save () {
			...
			.then((response) => {
				...
				if(response.data.Body !== null){
					setTimeout(function(){
						app.$router.push({name:"voters-view",params:{id:response.data.Body}})
					},1000)
				} else {
					app.isSave = true;
				}
