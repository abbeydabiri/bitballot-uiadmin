<template>
    <section>
      ...
      <table-display>
        ...
        <template slot="tableBody" v-if="recordList.length > 0">
          <tr class="stripe-dark" v-for="(user, index) in recordList" :key="index">
            <td class=" pa2"> 
              <span class="f7">#{{index+1}}.</span> {{user.Details}}
            </td>
            <td class=" pa2 dn dib-ns f7">{{user.Date}}</td>
            <td class=" pa2 tr pr2">
      ...
      <div v-if="recordList.length < 1">
        <h1 class="black f6 tc ma5">There are currently no users within the system</h1>
      </div>
