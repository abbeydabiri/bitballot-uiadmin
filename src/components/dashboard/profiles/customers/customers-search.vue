<template>
    <section>
      <div class="w-100 pa2 bg-light-gray br4 cf inline-flex items-center relative">
        ...
        <router-link :to="{'name':'customers-new'}" class="fl right-1 absolute ph2 pv1 br4 bg-red hover-bg-dark-red grow-ns pointer f6 white tc no-underline">
          + New
        </router-link>
      </div>
      ...
      <table-display>
        ...
        <template slot="tableBody" v-if="recordList.length > 0">
          <tr class="stripe-dark" v-for="(customer, index) in recordList" :key="index">
            ...
            <td class=" pa2 f7">{{customer.Phone}}</td>
            <td class=" pa2 f7">{{customer.Email}}</td>
            <td class=" pa2 f7">{{customer.Address}}</td>
      ...
      <div v-if="recordList.length < 1">
        <h1 class="black f6 tc ma5">There are currently no customers record  within the system</h1>
      </div>
