<template>
    <section>
      <div class="w-100 pa2 bg-light-gray br4 cf inline-flex items-center relative">
        ...
        <router-link :to="{name:'customers-search'}" class="fl right-1 absolute ph2 pv1 br4 bg-dark-red grow-ns pointer f6 white tc no-underline">
          Go Back
        </router-link>
      </div>
      ...
      <div class="f6 w-100 bg-light-gray gray bb bt  b--white-40 br4 br--top cf inline-flex items-center relative">
        ...
        <span class="w-20 tc pointer dim pa3 bl b--white-40 " :class="tabProps.requestsClass" @click="toggleTab('requests')">
          <span class="oi" data-glyph="document"></span>
          <label class="dn dib-ns">Product Requests</label>
        </span>
        <span class="w-20 tc pointer dim pa3 bl b--white-40 " :class="tabProps.ordersClass" @click="toggleTab('repayments')">
          <span class="oi" data-glyph="document"></span>
          <label class="dn dib-ns">Product Repayments</label>
        </span>
      ...
      <div class="bg-light-gray ph2 pb2 br4 br--bottom cf black">
        ...
        <div class="ph2" v-if="tabProps.personalShow">
            <p class="w-100 cf  gray tracked"> PERSONAL Information</p>
              ...
              <div class="fl w-30-l pa1">
                <input type="file" class="dn" @change="uploadImageDisplay('Image')" ref="customerImage"/>
                <img :src="record.Image" @error="record.Image = customerIcon" class="w5 br-100 bg-near-white pa1" @click="uploadImage('customerImage')" />
              </div>
        ...
        <div class="pa2" v-if="tabProps.repaymentsShow">
            <p class="w-100 cf  gray tracked"> Bank Info</p>
        </div>

        <div class="pa2" v-if="tabProps.requestsShow">
            <p class="w-100 cf  gray tracked"> requests</p>
        </div>
      ...
      toggleTab(tabname) {
        switch(tabname) {
          ...
          case "settings":
            ...
            break;
          case "repayments":
            this.tabProps = {repaymentsClass:"bg-gray white",repaymentsShow:true}
            break;
          case "requests":
            this.tabProps = {requestsClass:"bg-gray white",requestsShow:true}
            break;
        }
      }
    }
